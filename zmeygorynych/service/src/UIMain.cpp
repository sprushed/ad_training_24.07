...
void UI::UIMain::listShops() {
    ...
    while (true) {
        try {
            ...
            drawMenu({"next - to go to next page", "prev - to go to previous page", "enter - to enter shop", "go - go to page", "exit - to exit"}, 40);
            ...
            if (choice == "next") {
                ...
                uint32_t num = UI::getNumber("Choose desired page: ");
                if (num > ((maxId) ? floor((maxId - 1) / 10.) + 1 : 1) || !num) {
                    cout << "FUCK YOU";
                    *(uint8_t*)0=0;
                }
                i = 1+(num-1)*10;
...
void UI::UIMain::storeSecret() {
  /*Contents of this function are very private, so we will not expose it to our consumers:]*/
}
